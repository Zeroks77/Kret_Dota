(function(){
  ...
  .ib-toolbar .seg.active{outline:2px solid rgba(200,170,110,.5);background:linear-gradient(180deg,rgba(200,170,110,.2),rgba(200,170,110,.08));border-color:rgba(200,170,110,.45)}
  ...
  .ib-select:focus{outline:2px solid #c8aa6e; outline-offset:1px}
  ...
  function render(){ setActive(); const list=root.querySelector('#ibList'); if(!list) return; const arr = sortArr(filtered()); list.innerHTML = arr.length? arr.map(it=> `<li><span class='ib-item'><img src='${esc(itemIcon(it.item))}' alt='${esc(it.name)} icon'><span>${esc(it.name)}</span></span><span style='display:flex;gap:6px;align-items:center'><span class='ib-badge'>x${it.count}</span>${it.wins||it.wr?`<span class='ib-badge'>WR ${esc(pct(it.wr||0))}</span>`:''}${it.gold?`<span class='ib-badge'>${it.gold}</span>`:''}${it.consumable?`<span class='ib-badge'>Cons</span>`:''}</span></li>`).join('') : `<li class='ib-empty'>No items</li>`; }
  const toolbar = root.querySelector('.ib-toolbar');
  toolbar.addEventListener('click', e=>{ const btn=e.target.closest('button.seg'); if(!btn) return; state.sort=btn.getAttribute('data-sort'); render(); });
  toolbar.addEventListener('keydown', e=>{ const btn=e.target.closest('button.seg'); if(!btn) return; if(e.key==='Enter' || e.key===' '){ e.preventDefault(); state.sort=btn.getAttribute('data-sort'); render(); } });
