(function(){
  ...
  root.querySelector('.ld-toolbar').addEventListener('click', e=>{ const btn=e.target.closest('button.seg'); if(!btn) return; if(btn.hasAttribute('data-lane')) state.lane=btn.getAttribute('data-lane'); if(btn.hasAttribute('data-sort')) state.sort=btn.getAttribute('data-sort'); persist(); render(); });
